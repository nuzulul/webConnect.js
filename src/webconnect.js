...
export class Webconnect{
	...
	Send(payload,connectinput){
		this.fsendData("",payload,connectinput.connectId,connectinput.metadata)
	}
	...
	openStreaming(input,connectinput){
		let peerId = connectinput.connectId
		if(peerId == null){
			this.connectpeers.forEach((peer,index)=>{
				let engine = peer.engine
				if(engine.includes("torrent")){
					this.TORRENT.addStream(input, peer.id)
				}
				else if (engine.includes("mqtt")){
					this.MQTT.addStream(input, peer.id)
				}
				else if (engine.includes("ipfs")){
					this.IPFS.addStream(input, peer.id)
				}
			})
		}else{
			const searchPeer = this.connectpeers.findIndex((peer) => peer.id==peerId)
			if(searchPeer > -1){
				let engine = this.connectpeers[searchPeer].engine
				if(engine.includes("torrent")){
					this.TORRENT.addStream(input, peerId)
				}
				else if (engine.includes("mqtt")){
					this.MQTT.addStream(input, peerId)
				}
				else if (engine.includes("ipfs")){
					this.IPFS.addStream(input, peerId)
				}
			}
		}
	}
	...
	onStream = () => {}
	incomingStreaming = f => (this.onStream = f)
	
	closeStreaming(input,connectId){
		let peerId = connectinput.connectId
		if(peerId == null){
			this.connectpeers.forEach((peer,index)=>{
				let engine = peer.engine
				engine.forEach((protocol,idx)=>{
						if(protocol == "torrent")this.TORRENT.removeStream(input, peer.id)
						if(protocol == "mqtt")this.MQTT.removeStream(input, peer.id)
						if(protocol == "ipfs")this.IPFS.removeStream(input, peer.id)
				})
			})	
	...
	static init({
		...
		const config = {appId: appName,password:"Browser to browser connection without server",rtcConfig:rtc}
